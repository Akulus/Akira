import { Message, Role, VoiceChannel } from 'eris';
        ...
        async (msg: Message) => {
            ...
            if(client.player.canBypassVoting(msg)) {
                ...
                if(serverQueue.songs[1]) {
                    msg.channel.createMessage(`🎵 **Skipped to:** ${serverQueue.songs[1].title} \`[${client.player.getReadableTimestamp(serverQueue.songs[1].duration)}]\``);
                }
                else {
                ...
                if(serverQueue.songs[0].votesToSkip.includes(msg.author.id)) return msg.channel.createMessage(`📋 You already voted to **skip** current song. ┇ ${client.player.getVotingProgressBar(serverQueue.songs[0].votesToSkip.length, required)}`);
                ...
                if(serverQueue.songs[0].votesToSkip.length >= required) {
                    ...
                    return msg.channel.createMessage(`📋 **Voted** to skip the current song. ┇ ${client.player.getVotingProgressBar(serverQueue.songs[0].votesToSkip.length, required)}`);
                }
            }
        },
        {
