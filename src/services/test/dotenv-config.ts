...
export default async function validateConfig(): Promise<void> {
    console.log(`Running ${chalk.bold('dotenv-config.ts')} test script. This code will check .env file and generate config object.\n`);

    let testParam: string | string[] | number | undefined | unknown = undefined;
    ...
    if (testParam && testParam !== '' && typeof testParam === 'string' && testParam.length > 15) {
        try {
            const youtube: Youtube = new Youtube(testParam);
            const video: unknown = await youtube.searchVideos('Aeden - Holding On', 1);
            if (!video || video === '') {
...
function success(content: string): void {
    return console.log(`${chalk.greenBright.bold('[SUCCESS]')} ${content}`);
}
...
function fail(content: string): void {
    console.log(`${chalk.redBright.bold('[FAIL]')} ${content} Check your [.env] file.`);
 ...
 * @returns {void}
 */
function unknown(content: string): void {
    return console.log(`${chalk.yellow.bold('[UNKNOWN]')} ${content}`);
}
